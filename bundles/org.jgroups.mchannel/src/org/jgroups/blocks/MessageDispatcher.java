 ...
 *
 * @author Bela Ban
 * @version $Id: MessageDispatcher.java,v 1.1 2009/07/30 00:58:11 phperret Exp $
 */
public class MessageDispatcher implements RequestHandler {
    ...
    public MessageDispatcher(MChannel channel2, MessageListener l,	MembershipListener l2) {
    	this.mchannel=channel2;
        prot_adapter=new ProtocolAdapter();
        if(mchannel != null) {
            local_addr=mchannel.getLocalAddress();
        }
        setMessageListener(l);
        setMembershipListener(l2);
        if(mchannel != null) {
        	channel=((MChannelImpl)mchannel).getChannel();
            channel.setUpHandler(prot_adapter);
        }
        start();
    }
    ...
    public MessageDispatcher(MChannel channel, MessageListener l, MembershipListener l2, RequestHandler req_handler) {
        this(channel, l, l2);
        setRequestHandler(req_handler);
    }
